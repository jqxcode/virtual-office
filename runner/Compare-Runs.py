    ...
    for r in bug_reports:
        ...
        if total_bugs > 0 and needs_human == total_bugs:
            issues.append({
                ...
                "title": f"[Tool Issue] All {total_bugs} bugs returned needs_human on {r['gen_date'][:10] if r.get('gen_date') else 'latest'}",
                "body": (
                    f"## Problem\n\n"
                    f"All **{total_bugs}** bugs in the latest run were classified as `needs_human` "
                    f"with zero routed or fix-identified results.\n\n"
                    f"This suggests the agent lacks sufficient telemetry access or code search "
                    f"capability to make actionable determinations.\n\n"
                    f"## Bug details\n\n"
                    + "\n".join(
